...
class UcAction extends BaseAction {
	...
	public function add() {
		...
		if (isset ( $_POST ['submit'] )) {
			...
			// 下载远程图片
			if (C ( 'down_status' ) == 1) {
				$data ['img'] = $this->down ( $data ['img'], $data ['item_key'] );
			}
			
			// 保存上传图片
			if ($_FILES ['upload_img'] ['name'] != '') {
				$type = explode ( '/', $_FILES ['upload_img'] ['type'] );
				$_FILES ['upload_img'] ['name'] = md5 ( $_FILES ['upload_img'] ['name'] ) . '.' . $type [1];
				$dir = date ( "Ymd" );
				$upload_info = $this->upload_img ();
				$this->downitem ( $upload_info [0] ['savepath'] . $upload_info [0] ['savename'], $upload_info [0] ['savename'] );
	...
	private function match_url($url) {
		$url = urldecode ( $url );
	...
	private function upload_img() {
		...
		$upload->savePath = './Uploads/LocalItems/' . $dir . '/'; // 设置附件上传目录
	...
	public function get_tags($title) {
		...
		$pscws = new PSCWS4 ();
		$pscws->set_dict ( './Public/statics/scws/dict.utf8.xdb' );
		$pscws->set_rule ( './Public/statics/scws/rules.utf8.ini' );
