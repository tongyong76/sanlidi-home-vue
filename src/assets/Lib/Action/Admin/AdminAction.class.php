...
class AdminAction extends BaseAction{
	...
	public function index(){
		...
		$count = $admin_mod->where('is_del=0 and role_id<>0')->count();
		$page = new Page($count,10);
		$admin_list = $admin_mod->where('is_del=0 and role_id<>0')->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function add(){
		...
		if(isset($_POST['submit'])){
			...
			if($result){
				$this->error('管理员'.$_POST['username'].'已经存在');
			}
