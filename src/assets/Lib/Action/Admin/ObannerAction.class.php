<?php
class ObannerAction extends BaseAction{

	//分页显示所有商品
	public function index(){
		$pid = $_REQUEST['pid'];
		$bannerMod=M('banner_other');
		$list = $bannerMod->where('is_del=0 and pid='.$pid)->order('ordid desc')->select();
	...
	public function add(){
		...
		if($_POST['submit']){
			...
			$bannerMod->add($data);
			$this->success('添加成功',U('Obanner/index'));
		}else{
...
//		}
	...
	public function edit(){
		...
		if($_POST['submit']){
			...
			$this->success('修改成功',U('Obanner/index',array('pid'=>$data['pid'])));
		}else{
