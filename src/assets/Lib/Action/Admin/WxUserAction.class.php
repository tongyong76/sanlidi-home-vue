...
class WxUserAction extends BaseAction{
	...
	public function index(){
		...
		if(isset($_GET['keyword'])){
			...
			$where.="wx_nickname like '%$keyword%'";
		}
	
		$count = $user_mod->where($where)->count();
		$page = new Page($count,20);
		$show = $page->show();
		$list=$user_mod->where($where)->order("last_time desc,wx_id desc")->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function edit(){
		...
		if (isset($_POST['submit'])) {
			...
			$result = $user_mod->where("wx_id=" . $data['wx_id'])->save($data);
			if(false !== $result){
				$this->success('修改成功！',U('WxUser/index'));
			}else{
				$this->error('修改失败！');
			}
		} else {
			...
			$id=$_GET['id'];
			$info = $user_mod->where('wx_id='.$id)->find();
