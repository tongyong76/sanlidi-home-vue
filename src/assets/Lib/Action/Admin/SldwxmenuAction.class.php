<?php
class SldwxmenuAction extends Action{

	public function index(){
		$mod = M('wx_menu_sanlidi');
	...
	public function add(){
		...
		if ($_POST['submit']){		
			...
			$row = $mod->add($data);
			if ($row){
				$this->success('添加成功！',U('Wxmenu/index'));
			}else {
			...
			$data = $mod->order('ordid desc,id desc')->where("is_del=0 and floor<>3")->select();
			$menu = arrToMenu($data,0); 
	...
	public function edit(){
		...
		if ($_POST['submit']){
			...
			if ($_FILES['cimg']['name'] != '') {
				...
				$data['cimg'] = '/Uploads/goods/s_'. $upload_info['0']['savename'];
			}
			$data['floor'] = $mod->where('id='.$data['pid'])->getField('floor')+1;
			$save=$mod->where("id=$id")->save($data);
			$this->success('修改成功！',U('Wxmenu/index'));
