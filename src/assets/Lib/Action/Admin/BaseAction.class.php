...
class BaseAction extends Action {
	public function _initialize() {
		...
		//判断是否登录
		if ($_COOKIE['check']){
			$check=md5($_COOKIE['username'].C('login_key'));
			if ($_COOKIE['check'] != $check){
				setcookie('username','',time()-1,'/');
				setcookie('check','',time()-1,'/');
				$this->error('非法用户，请重新登录',U('Signin/index'));
			}else {
				//get_active_plugins();
				$this->assign("uid",$_COOKIE['id']);
				return true;
			}
		}else {
			$this->error('请登录',U('Signin/index'));
		}
	}
    ...
    public function uploadFile($savePath) {
    	...
    	import("ORG.Net.UploadFile");
    	$upload = new UploadFile();
    	$upload->maxSize  = 4197152;// 设置附件上传大小
    	$upload->savePath = $savePath;// 设置附件上传目录
    	$upload->saveRule = uniqid;
    	$upload->allowExts  = array('xls','xlsx','doc','docx','rar','zip');// 设置附件上传类型
