...
class Page {
    ...
    public function show() {
        ...
        if($this->url){
			...
			//翻页式样修改
			if(MODULE_NAME == 'Ask' && ACTION_NAME == 'tags'){
				$url="__ROOT__/ask/tags-".$parameter['id']."-".__PAGE__.".html";
			}
			
			//$url .= "-".$parameter['id']."-__PAGE__/";
        }
        //上下翻页字符串
        $upRow          =   $this->nowPage-1;
        $downRow        =   $this->nowPage+1;
        if ($upRow>0){
            $upPage     =   "<a style='width:42px;' href='".str_replace('__PAGE__',$upRow,$url)."'>".$this->config['prev']."</a>";
        }else{
            $upPage     =   '';
        }
        if ($downRow <= $this->totalPages){
            $downPage   =   "<a style='width:42px;' href='".str_replace('__PAGE__',$downRow,$url)."'>".$this->config['next']."</a>";
        }else{
            $downPage   =   '';
        }
        // << < > >>
